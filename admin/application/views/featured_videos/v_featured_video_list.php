...
<?php echo $menu;?><aside class="right-side">
			...
			<section class="content-header">
				...
				<span><?php echo lang('featured_videos');?></span>
				<ol class="breadcrumb">
					<li><a href="<?=site_url('home');?>"><?php echo lang('home');?></a></li>
                                        <li class="active"><?=lang('featured_video_list');?></li>                                     
							...
							<div class="grid-body">
								<table id="dataTables1" class="table table-hover display" cellspacing="0" width="100%">
									<thead>
										<tr>
											...
											<th><?php echo lang('video_type');?></th>
                                                                                        <th><?php echo lang('featured_status');?></th>
									...
									<tbody>
                                                                           ...
                                                                           if(isset($featured_video_details))
                                                                                { 
                                                                                 
                                                                                    foreach ($featured_video_details as $results):?>
										...
										<tr id="<?php echo $results->video_id;?>" >
                                                                                ...
                                                                                $statusChecked = '';
                                                                                if(isset($results->video_highlight) && ($results->video_highlight == 1)){
                                                                                    $statusChecked = ' checked';
                                                                                }
                                                                                    ?>
                                                                                  <td>
                                                                                       <span style="display:none;"><?=$results->video_highlight;?></span>
                                                                                    <input type="checkbox" class="js-switch xsmall" id="<?php echo $results->video_id;?>" <?php echo $statusChecked;?> onchange="changeStatus(<?php echo $results->video_id;?>,'<?php echo $results->video_highlight;?>')"/></td>	
 ...
 <script>
    // changing video status
        function changeStatus(chkid,chkstatus)
        {
           ...
           $.ajax({
               url: baseurl+'featured_videos/set_featured_status',
               type: 'POST', 
